...
const columns = [
  ...
  { key: "CurrentSale", label: "Current Sale" },
  { key: "HandoverAmount", label: "Handover Amount" },
  { key: "ClosingAmount", label: "Closing Amount" },
];

export default function RetailerProjectionHistoryDialog({
  userId,
  userName,
  onClose,
}) {
  ...
  useEffect(() => {
    ...
    const loadRows = async () => {
      try {
        setLoading(true);
        setError("");
        const data = await apiBase.getRetailerProjectionAmountLast30Days(userId);
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
      ...
      const safeUserId = (userId || "retailer").replace(/[^\w-]+/g, "-");
      const dateLabel = new Date().toISOString().slice(0, 10);

      link.href = url;
      link.download = `retailer-projection-history-${safeUserId}-${dateLabel}.xlsx`;
    ...
    <div className="app-modal-overlay" onClick={onClose}>
      ...
      >
        <div className="app-modal-header">
          <div>
            <h2 className="app-modal-title">Retailer Projection Last 30 Days</h2>

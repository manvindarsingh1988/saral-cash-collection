...
export default function UserProfileMenu({ user, onSignOut, onForgotPassword }) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {showMenu && (
        <div className="absolute right-0 z-50 mt-2 w-44 rounded-2xl border border-slate-200/80 bg-white/95 shadow-lg backdrop-blur">
          <button
            onClick={() => {
              setShowMenu(false);
              navigate("/change-password");
            }}
            className="w-full rounded-t-2xl px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100"
          >
            Change Password
          </button>
          <button
            ...
            className="w-full rounded-b-2xl px-4 py-2 text-left text-sm text-red-600 hover:bg-gray-100"
          >
            Sign Out
          </button>
        </div>
